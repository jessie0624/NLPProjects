...
class MyDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if 'bert' in self.model_name:
            # 如果是bert 类型, 使用tokenizer 的 encode_plus方法处理
            ...
            input_ids, attention_mask, token_type_ids = text_dict['input_ids'],\
                        text_dict['attention_mask'], text_dict['token_type_ids']
        else:
            # 如果是cnn, rnn, transformers 则使用自建的dictionary 来处理
            text = text.split()
            text = text + [0] * max(0, self.max_length - len(text)) \
                if len(text) < self.max_length else text[:self.max_length]
...
def collate_fn(batch):
    ...
    def padding(indice, max_length, pad_idx=0):
        """
        pad函数, token type id 右侧pad 0
        """
        pad_idince = [
            item + [pad_idx] * max(0, max_length - len(item)) for item in indice ]
        return torch.tensor(pad_idince)
