...
def main(train_file, dev_file, target_dir, \
         epochs=20, batch_size=64, lr=2e-05, \
...
if __name__ == '__main__':
    ...
    Path(root_path / "data" /"ranking3").mkdir(parents=True, exist_ok=True)
    Path(root_path / "model" /"ranking3").mkdir(parents=True, exist_ok=True)

    ranking_bert_train = os.path.join(root_path, 'data','ranking3', 'train.csv')
    ranking_bert_dev = os.path.join(root_path, 'data','ranking3', 'dev.csv')
    rank_path = os.path.join(root_path, "model","ranking3")
