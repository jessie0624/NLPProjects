...
def read_file(path, is_train=False):
    ...
    with path.open(mode='r') as f:
        tmp = []
        sessions = set()
        session_id, culiststom_id, is_assistance, content = '', '', '', []
        for lines in f:
            ...
            if session_id != session_id_in_doc and session_id != '':
                ...
                if fc != "":
                    tmp.append([
                        session_id, 'custom' if str(is_assistance) == '0'
                        else 'assistance',
                        fc
                    ])
                    content = []
...
def generate_data(filepath, save, to_file, pair):
    ...
    @param:
        - filepath:　原始数据路径
        - save: 是否保存
        - to_file: 保存文件名，会根据文件名判断是否为训练集
        - pair: 是否生成问答pair的结果
    @return: 处理后可作为模型输入的csv数据集        
    """
    if isinstance(to_file, Path):
        train_flag = True if 'train' in to_file.name else False

    data = read_file(filepath, train_flag)
    ...
    data = data.pivot_table(index=['index', 'session_id'], columns='role',
