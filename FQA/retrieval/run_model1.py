...
def read_corpus():
    ...
    qList_kw = [] #问题的关键词列表
    aList = []
    data = pd.read_csv(config.retrieval_data)[:100000] #取前1000做demo
...
if __name__ == '__main__':
    ...
    if eval_flag:
        ## evaluate: 
        eval_questions = list(data.iloc[:1000]['custom'])
        y_pred = ss.get_recall_result(eval_questions, 10)
        recall, wrong_index = ss.recall_topk(y_pred, list(range(1000)), 10)
