...
from rank_utils.data_utils import (PROCESSORS, multi_classification_convert_examples_to_dataset,
                               compute_metrics)
from rank_utils.adv_utils import FGM
sys.path.append('..')
from utils.tools import create_logger
from config import root_path, rank_path, \
...
def train(args, tokenizer, model):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.adv_type == 'fgm':
                fgm.attack()  ##对抗训练
                loss_adv = model(**inputs)[0] # 对输出进行对抗训练
                loss_adv.backward()
                fgm.restore()
...
def main():
    ...
    parser.add_argument(
        "--model_name_or_path",
        default=os.fspath(config.model_name_or_path),
        type=str,
    ...
    parser.add_argument(
        "--data_dir",
        default=os.fspath(conifg.r2_data_dir),
        type=str,
        help="Path to data ",
    )
    parser.add_argument(
    ...
    parser.add_argument(   
        "--output_dir",
        default=os.fspath(config.r2_output_dir),
        type=str,
    ...
    parser.add_argument(
        "--num_train_epochs", default=config.num_train_epochs, type=float, help="Total number of training epochs to perform.",
    )
    parser.add_argument(
        "--max_steps",
        default=-1,
